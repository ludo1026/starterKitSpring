...
<%-- Created on 26 nov. 2013 ( Time 16:07:58 )  --%>
...
<head>
	...
	<title>WorkgroupStore</title>
		...
		var urlBase = '<c:url value="/workgroup" />';
				...
				<tbody>
					<c:forEach items="${listWorkgroups}" var="workgroup">
						<tr>
							...
							<td>${workgroup.creationDate}</td>
							<c:url var="editWorkgroupUrl" value="/workgroup/edit/${workgroup.id}"/>
							<td><a class="btn btn-info" href="${editWorkgroupUrl}">Edit</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
